...
def parseStandardFrame(frameData):
    ...
    for i in range(numTLVs):
        ...
        # Presence Detection in each zone
            ...
            # Assuming parseEnhancedPresenceInfoTLV returns a list or dictionary with zone presence data
            presenceData = outputDict['enhancedPresenceDet']

            # print(presenceData)

            # try:
            #     # Example: iterate through zones and print detections
            #     for zone_id, is_present in enumerate(presenceData):
            #         if is_present:
            #             print(f"Object detected in Zone {zone_id + 1}")
            # except:
            #     print("Failed to get Zone IDs")
